...
OP_KW_LIST = [
    ...
    ("Шестерни/зубчатые",    ["зубофрез", "зубонарез", "зубодолб", "зубошлиф", "шестерн", "зубчат", "шлицефрез", "gear"]),
    ("Кузнечная/ковка",      ["ковк", "кузнечн", "горячештамп", "forging"]),
    ("Гидроабразивная резка", ["гидроабразив", "waterjet", "гидрорезк"]),
    ("Лазерная гравировка/маркировка", ["гравиров", "маркировк", "engraving"]),
    ("3D-печать/аддитивное",  ["3d печат", "3д печат", "аддитивн", "slm", "sls", "fdm"]),
    ("Напыление/металлизация", ["напылени", "газотермическ", "металлизаци", "наплавк износо"]),
    ("Резка/заготовительная", ["ленточнопил", "рубка металл", "раскрой лист", "гильотин", "ленточн пил"]),
    ("Оснастка/штампы/пресс-формы", ["пресс-форм", "пресс форм", "оснастк под заказ", "штамповая оснастк", "формообразующ"]),
    ("Металлоконструкции",    ["металлоконструкц", "металлокаркас", "сварн конструкц", "резервуар", "ёмкост", "емкост"]),
    ("РТИ/резинотехника",     ["рти", "резинотехн", "резинометалл", "вулканизац", "формов резин"]),
    ("Пружины/навивка",       ["пружин", "навивк", "рессор"]),
    ("Хонингование/притирка", ["хонингов", "притир", "суперфиниш", "полировк", "доводк"]),
    ("Пескоструйная",         ["пескоструй", "дробеструй", "sandblast"]),
    ("Измерение/контроль (КИМ)", ["кмм", "координатно-измерит", "метролог", "cmm", "контрол качеств"]),
]


@app.get("/api/operations")
def api_operations(all: int = 0):
    """
    Возвращает операции для Mini App.
    По умолчанию (all=0) — топ операций С КАРТОЧКАМИ (count>0) для фильтр-чипов CRM.
    all=1 — ПОЛНЫЙ список видов услуг (вкл. без карточек) для пикера размещения/профиля
    (иначе новые ниши без кооператоров не выбрать). Ищет ключи в полях возможностей.
    """
    import re as _re
    OP_KEYS = OP_KW_LIST
    ...
    for op_label, kws in OP_KEYS:
        ...
        if cnt > 0:
            counts[op_label] = cnt
    if all:
        # Полный список для пикера (порядок как в OP_KW_LIST), с фактическими счётчиками
        result = [{"op": lbl, "count": counts.get(lbl, 0)} for lbl, _ in OP_KEYS]
    else:
        # Только операции с карточками, по убыванию — для фильтр-чипов
